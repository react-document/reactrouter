...
这个问题通常源于你正在使用的类组件不支持钩子这一事实。在React Router v6 中，我们完全采纳钩子，并利用钩子去共享路由的所有内部状态。但是，这并不意味着你不能使用路由。假设你能够使用钩子(你采用的是React 16.8+)，你需要的只是一个封装方法。
...
而且，`Route`  v5 的渲染 API 特别多。当我们基于 v4 或者5开发的时候，我们通常会有下面的对话：
